...
| #TBD | Kobo cover aspect-ratio padding: pads book covers to the device's native aspect ratio (Libra Color/2 1264×1680 default; Clara family alt) before serving over the Kobo sync API, so devices stop white-letterboxing tall publisher covers on the home and sleep screens. Five fill modes — edge_mirror (default; mirrors outer edges into the pad area for a natural artwork extension), edge_blur (soft bokeh border), most-common color, average color, manual hex. Padded variants are cached in the existing `/config/thumbnails/` dir keyed by `book_uuid + resolution + src_mtime + settings_hash`; `CoverImageId` includes `-p<settings_hash>` so devices re-fetch on settings change. Off by default; new admin section under Configuration → Kobo sync. Original cover files on disk are never modified. See `notes/KOBO-COVER-PADDING-DESIGN.md`. | `TBD` | TBD |
